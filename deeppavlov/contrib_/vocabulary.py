...
=======
    """
    def __init__(self, source: Union[str, tf.Tensor]) -> None:
        if isinstance(source, str):
            self.lookup_table = tf.contrib.lookup.index_table_from_file(source, default_value=0)
            self.reverse_lookup_table = tf.contrib.lookup.index_to_string_table_from_file(source, default_value='<UNK>')
        elif isinstance(source, tf.Tensor):
            self.lookup_table = tf.contrib.lookup.index_table_from_tensor(source, default_value=0)
            self.reverse_lookup_table = tf.contrib.lookup.index_to_string_table_from_tensor(source, default_value='<UNK>')
